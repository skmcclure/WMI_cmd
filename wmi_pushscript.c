...
static void parse_args(int argc, char *argv[], struct program_args *pmyargs)
{
    ...
    poptSetOtherOptionHelp(pc, "//host\n\nExample: wmi_rexec -U [domain/]adminuser%password //host <script-file> <remote-file-location>");
    ...
    if (argc_new < 2 || argv_new[1][0] != '/' || argv_new[1][1] != '/')
    {
        poptPrintUsage(pc, stdout, 0);
        poptFreeContext(pc);
        exit(1);
...
WERROR WBEM_WaitForProcessExit(struct IWbemServices *pWS,
                               ...
                               uint32_t pid, uint32_t *exitstatus)
{
    TALLOC_CTX *ctx;
    union CIMVAR v;
    WERROR result;
                          ...
                          int wait_for_complete)
{
    ...
    if (wait_for_complete)
    {
        ...
        result = WBEM_WaitForProcessExit(pWS, pEnum, pid, &exitstatus);
...
WERROR process_file(char *scriptName, char *remoteFileName,
                    struct IWbemServices *pWS)
{
    ...
    // For the first line, we want to use > to overwrite any existing file.
    redirect = overwrite;

    while(1)
    { 
        char scriptLine[STRING_SIZE];
        char command[STRING_SIZE];
        
        bzero(command, STRING_SIZE);
        bzero(scriptLine, STRING_SIZE);
        if (fgets(scriptLine, STRING_SIZE, scriptFile) == NULL)
        {
            if (feof(scriptFile))
                break;
            
            fprintf(stderr, "ERROR: Unable to read line from %s\n", scriptFile);
            exit(1);
        }
        ...
        if (len == 0)
        {
            ...
            snprintf(command, STRING_SIZE,
                     "c:\\windows\\system32\\cmd.exe /C ECHO. %s %s",
                     redirect, remoteFileName);
        }
        else
        {
            // Regular line
            snprintf(command, STRING_SIZE,
                     "c:\\windows\\system32\\cmd.exe /C ECHO %s %s %s",
                     scriptLine, redirect, remoteFileName);
        }

        // After the first line, switch to appending.
        redirect = append;
        
        // Create the process
        DEBUG(0, ("Echo: %s\n", scriptLine));
        result = WBEM_RemoteExecute(pWS, command, 1);
...
int main(int argc, char **argv)
{
    ...
    result = process_file(args.scriptfile, args.remotefile, pWS);
